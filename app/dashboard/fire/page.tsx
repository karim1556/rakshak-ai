...
import { ArrowLeft, Flame, AlertTriangle, MapPin, Loader2, UserCheck, Clock, Navigation, X, Radio, RotateCcw, Route } from 'lucide-react'
...
function FireContent() {
  ...
  const load = async () => {
    ...
    const { data: byType } = await supabase.from('incidents').select('*').in('type', ['fire', 'accident']).neq('status', 'resolved').order('created_at', { ascending: false })

    // Also get incidents assigned to fire/rescue responders (dispatched from dispatch team)
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        const { data: fireResp } = await supabase.from('responders').select('id').in('role', ['fire', 'rescue'])
        if (fireResp) {
          for (const r of fireResp) {
  ...
  const markers = [
    ...incidents.filter(i => i.location_lat && i.location_lng).map(i => ({
      ...
      popup: `<b>📍 ${i.summary || 'Fire Incident'}</b><br/><span style="color:#dc2626;font-weight:600">${i.severity}</span> · ${i.type}<br/><em style="color:#6366f1">Live Location</em>`,
      type: 'user-live' as const
    })),
    ...responders.filter(r => r.location_lat && r.location_lng).map(r => {
      // Calculate distance to the nearest incident
      let distInfo = ''
  ...
  const handleAssign = async (incidentId: string, responderId: string) => {
    // Place responder near the incident location (realistic nearby position)
    const inc = incidents.find(i => i.id === incidentId)
    const locUpdate: any = { status: 'busy', current_incident_id: incidentId }
    if (inc?.location_lat && inc?.location_lng) {
      // Random offset 0.005-0.015 degrees (~0.5-1.5km)
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-slate-50 via-white to-orange-50/30 text-slate-900 flex flex-col">
      {/* Assign Modal */}
      {assignModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setAssignModal(null)}>
          <div className="w-full max-w-sm bg-white border border-slate-200 rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-orange-100 bg-orange-50 rounded-t-2xl">
            ...
            <div className="p-3 max-h-64 overflow-y-auto space-y-1.5">
              {availableResponders.length === 0 ? (
                ...
                <button
                  ...
                  className="w-full p-3 bg-white border border-slate-200 rounded-xl hover:border-orange-300 hover:bg-orange-50/50 transition-all flex items-center gap-3 group"
                >
                  ...
                  <UserCheck className="h-4 w-4 text-slate-300 group-hover:text-orange-500 transition-colors" />
                </button>
              ))}
      ...
      <header className="h-14 border-b border-slate-200/60 flex items-center justify-between px-5 flex-shrink-0 bg-white/70 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold">Fire Command</span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-64 border-r border-slate-200/60 flex flex-col flex-shrink-0 bg-white/50">
          <div className="grid grid-cols-2 gap-2 p-3 border-b border-slate-100">
            <div className="bg-orange-50 rounded-xl p-3 text-center border border-orange-100">
              ...
              <p className="text-[9px] text-orange-400 uppercase tracking-wider font-semibold">Active</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <button
                ...
                className={`w-full p-3 text-left border-b border-slate-100 transition-colors ${selected?.id === inc.id ? 'bg-orange-50/70 border-l-2 border-l-orange-500' : 'hover:bg-slate-50 border-l-2 border-l-transparent'}`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <AlertTriangle className={`h-3 w-3 ${inc.severity === 'CRITICAL' ? 'text-red-500' : 'text-orange-500'}`} />
                  <span className="text-xs font-semibold truncate flex-1 text-slate-800">{inc.summary || 'Fire Incident'}</span>
                </div>
                <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex flex-col">
          <div className="h-72 border-b border-slate-200/60 relative">
            ...
            {routes.length > 0 && (
              <div className="absolute top-2 right-2 z-[1000] bg-white/90 backdrop-blur-sm border border-orange-200 rounded-lg px-2.5 py-1.5 shadow-sm">
                <div className="flex items-center gap-1.5 text-[9px] text-orange-600 font-semibold">
            ...
            {selected ? (
              <div className="space-y-4 max-w-xl">
                <div className="flex items-start justify-between">
                  <div>
                    <h2 className="text-sm font-bold text-slate-900">{selected.summary || 'Fire Incident'}</h2>
                  ...
                  <div className="flex gap-1.5">
                    ...
                    {(selected.status === 'active' || selected.status === 'assigned') && (
                      <button onClick={() => setAssignModal(selected.id)} className="text-[10px] px-3 py-1.5 bg-orange-50 text-orange-600 rounded-lg hover:bg-orange-100 transition-colors font-semibold border border-orange-200 shadow-sm">
                ...
                {selected.risks?.length > 0 && (
                  <div>
                    <p className="text-[10px] text-slate-400 mb-1.5 font-semibold uppercase tracking-wider">Hazards</p>
                    ...
                    <ol className="space-y-1.5">
                      {selected.steps.map((s: string, i: number) => (
                        <li key={i} className="text-xs text-slate-600 flex items-start gap-2">
                          <span className="w-5 h-5 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center text-[9px] font-bold flex-shrink-0 mt-0.5">{i + 1}</span>
                          {s}
                        </li>
                      ))}
                    </ol>
